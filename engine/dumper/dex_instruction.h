...
#ifndef _ART_DEX_INSTRUCTION_H_
#define _ART_DEX_INSTRUCTION_H_
...
class Instruction
{
  public:
    ...
    // The following methods return the vB operand for all instruction formats
    // where it is encoded in the first 16 bits of instruction. The "inst_data"
    // parameter holds these 16 bits. The returned value is decoded from it.
    ...
    // Returns the opcode field of the instruction. The given "inst_data"
    ...
    // Returns true if the instruction allows control flow to go to the
    ...
    // Get the dex PC of this instruction as an offset in code units from the beginning of insns.
